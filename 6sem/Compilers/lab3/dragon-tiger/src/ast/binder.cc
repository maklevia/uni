...
void Binder::visit(Let &let) {
  ...
  for (size_t i = 0; i < decls.size(); ++i) {
    if (dynamic_cast<FunDecl *>(decls[i])) {
      size_t j = i;
      // Pass 1: Enter all functions in the group
      while (j < decls.size() && dynamic_cast<FunDecl *>(decls[j])) {
        FunDecl *f = static_cast<FunDecl *>(decls[j]);
        set_parent_and_external_name(*f);
        enter(*f);
        f->set_depth(functions.size());
        j++;
      }
      // Pass 2: Visit bodies of all functions in the group
      for (size_t k = i; k < j; ++k) {
        FunDecl *f = static_cast<FunDecl *>(decls[k]);
        push_scope();
        functions.push_back(f);
        for (auto param : f->get_params()) {
          param->accept(*this);
        }
        if (auto expr = f->get_expr()) {
          expr->accept(*this);
        }
        functions.pop_back();
        pop_scope();
...
void Binder::visit(Identifier &id) {
  ...
  if (vdecl->get_depth() < (int)functions.size() - 1) {
    if (!vdecl->get_escapes()) {
      vdecl->set_escapes();
      functions[vdecl->get_depth()]->get_escaping_decls().push_back(vdecl);
    }
  }
}
...
void Binder::visit(FunDecl &decl) {
  ...
  if (functions.empty()) { // main function
    decl.set_depth(0);
  } else {
    enter(decl);
