...
int main(int argc, char* argv[]) {
    ...
    const int TEST_SIZES[] = {12, 102, 498, 1002, 1500, 1998, 2502, 3000};
    ...
    if (ProcNum != GridSize*GridSize) {
        ...
        for (int i = 0; i < NUM_TESTS; ++i) {
            ...
            // ПЕРЕВІРКА: Чи ділиться розмір на GridSize
            if (Size % GridSize != 0) {
                if (ProcRank == 0) {
                    printf("Skipping size %d: Not divisible by GridSize (%d).\n", Size, GridSize);
                }
                continue; // Переходимо до наступного розміру в масиві
            }
